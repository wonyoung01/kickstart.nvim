...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      my_live_grep = function(opts, no_ignore)
        ...
        opts.attach_mappings = function(_, map)
          map({ 'n', 'i' }, '<C-h>', function(prompt_bufnr)
            local prompt = require('telescope.actions.state').get_current_line()
            require('telescope.actions').close(prompt_bufnr)
            my_live_grep({ default_text = prompt }, not no_ignore)
          end)
          return true
        end
      ...
      vim.keymap.set('n', '<leader>sg', my_live_grep, { desc = '[S]earch by [G]rep' })
      vim.keymap.set('n', '<leader>sd', builtin.diagnostics, { desc = '[S]earch [D]iagnostics' })
      vim.keymap.set('n', '<leader>sr', builtin.resume, { desc = '[S]earch [R]esume' })
      vim.keymap.set('n', '<leader>s.', builtin.oldfiles, { desc = '[S]earch Recent Files ("." for repeat)' })
      vim.keymap.set('n', '<leader><leader>', my_find_files, { desc = '[S]earch [F]iles (shortcut)' })
  ...
  },
  {
    ...
    config = function()
      ...
      vim.list_extend(ensure_installed, {
        ...
        'prettierd', -- Used to format various web development files (markdown, javascript, typescript, etc.)
      })
  ...
  { -- Highlight, edit, and navigate code
    ...
    branch = 'main', -- the rewrite; required for Neovim 0.11+
    lazy = false,
    build = ':TSUpdate',
    -- [[ Configure Treesitter ]] See `:help nvim-treesitter`
    -- The `main` branch has no `opts`/configs module. Parsers are installed
    -- via `require('nvim-treesitter').install(...)` and highlighting/indent are
    -- enabled per-buffer with `vim.treesitter.start()` in a FileType autocmd.
    config = function()
      ...
      vim.api.nvim_create_autocmd('FileType', {
        ...
        callback = function(args)
          ...
          -- latex is handled by vimtex, not treesitter (matches old `disable`).
          if ft == 'latex' or ft == 'tex' then
            return
          end
          -- start() throws if no parser is available for this buffer; ignore.
          if not pcall(vim.treesitter.start, args.buf) then
            return
          end
          -- Ruby relies on vim's regex indent; everything else uses treesitter.
          if ft ~= 'ruby' then
